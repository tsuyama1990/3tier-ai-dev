...
class TestTaskTreeAndDecomposition(unittest.TestCase):
    ...
    def test_add_task_root(self) -> None:
        ...
        self.assertEqual(node.task.task_id, "T-100")
    ...
    def test_add_task_with_parent(self) -> None:
        ...
        node = self.tree.add_task(subtask, parent_id="T-100")
        self.assertEqual(node.parent_id, "T-100")
        root = self.tree._nodes["T-100"]
    ...
    def test_decompose_adds_children(self) -> None:
        ...
        self.assertEqual(root.children[0].task.task_id, "T-100-S1")
        self.assertEqual(root.children[1].task.task_id, "T-100-S2")
    ...
    def test_execute_parallel_all_success(self) -> None:
        ...
        self.assertEqual(self.tree._nodes["T-100-S1"].status, "success")
        self.assertEqual(self.tree._nodes["T-100"].status, "success")
        self.assertIn("T-100-S1", results)
    ...
    def test_execute_parallel_partial_failure(self) -> None:
        ...
        self.assertEqual(self.tree._nodes["T-100-S1"].status, "success")
        self.assertEqual(self.tree._nodes["T-100-S2"].status, "failed")
        self.assertEqual(self.tree._nodes["T-100"].status, "failed")
    ...
    def test_get_summary_counts(self) -> None:
        ...
        self.assertEqual(summary["task_results"]["T-100"]["status"], "success")
        self.assertEqual(summary["task_results"]["T-100"]["retries"], 2)
    ...
    def test_parent_id_propagation(self) -> None:
        ...
        for sub in subtasks:
            self.assertEqual(sub.parent_task_id, "T-100")
    ...
    def test_run_epic_task_mcp_tool(self) -> None:
        ...
        # Clean up existing decisions mock path
        epic_dict = {
            "task_id": "T-200",
        ...
        try:
            ...
            self.assertEqual(res["epic_task_id"], "T-200")
